...
func _process(delta: float) -> void:
	...
	if rotation_speed <= 0.0:
		if spinning_audio.playing:
			spinning_audio.playing = false
	else:
		if not spinning_audio.playing:
			spinning_audio.playing = true
		spinning_audio.volume_db = log(remap(rotation_speed, 0.0, max_rotation_speed, 0.01, 1.0)) * 10 - 6.0
	
func is_rotating() -> bool:
	return is_equal_approx(rotation_speed, max_rotation_speed)

func _on_power_button_pressed(_button: Variant) -> void:
	(%PowerButtonAudio as AudioStreamPlayer3D).play()
